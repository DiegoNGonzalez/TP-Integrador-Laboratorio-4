...
public class ClienteDaoImpl implements ClienteDao {
	...
	public ArrayList<Cliente> listarClientesActivos() {
		...
		try (Connection conexion = Conexion.getConnection();
			...
			while (resultSet.next()) {
				...
				Nacionalidad nacionalidad = new NacionalidadDaoImpl()
	...
	public boolean bajaCliente(int idCliente) throws SQLException {
		...
		try (Connection conexion = Conexion.getConnection(); CallableStatement cst = conexion.prepareCall(query)) {
		...
		try (Connection conexion = Conexion.getConnection();
			...
			try (ResultSet resultSet = statement.executeQuery()) {
				...
				if (resultSet.next()) {
					...
					cliente.setDireccion(resultSet.getString("direccion"));
					cliente.setCuentas(
							negocioCuenta.obtenerCuentasPorCliente(Integer.parseInt(resultSet.getString("idCliente"))));
	...
	public int obtenerIdClientePorIdCuenta(int idCuenta) {
		...
		String query = "SELECT idCliente FROM cuentas where idCuenta= ?";
		int idCliente = -1;
